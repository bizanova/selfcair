...
class ReportsDesigner(models.Model):
    ...
    active = fields.Boolean(string='Active',
                            help="If unchecked, it will allow you to hide the report excel without removing it.",
                            default=True)
...
See bellow Legends for supported Date and Time Formats.
    ...
    template_name_id = fields.Many2one(comodel_name="ir.attachment", string="Template File Excel", required=False,
                                       ...
                                       domain="[('res_model', '=', 'reports.designer'), ('res_id', '=', id)]",
                                       copy=False
                                       )
    ...
    description = fields.Text('Displayed Description', translate=True,
                              help="This description is displayed in the wizard form when you print the report.")
    ...
    reports_designer_param_ids = fields.One2many('reports.designer.param', 'reports_designer_id',
                                                 string='Report Parameters', copy=False)
    report_id = fields.Integer(compute='_compute_id', string="Report id", store=False)
    reports_designer_param_content = fields.Char(compute='_compute_reports_designer_param_content',
                                                 string="Report Parameters ", store=False)
    reports_designer_section_ids = fields.One2many('reports.designer.section', 'reports_designer_id',
                                                   string='Report Sections', copy=True)
    ...
    def create_action(self):
        for report in self:
            ...
            action_id = self.env['ir.actions.act_window'].search(
                ["&", ('res_model', '=', 'reports_designer_wizard'), "|", ('context', '=', context_report1),
                 ('context', '=', context_report2)]).id
            if action_id:
            ...
            for param_id in res.reports_designer_param_ids:
                if param_id.wizard_param_ir_model_field_id.id == False:
                    ...
                    if param_id.param_ir_model_id.id != False:
                        vals_create["relation"] = self.env['ir.model'].search(
                            [('id', '=', param_id.param_ir_model_id.id)], []).model
                        if param_id.type_param == 'many2one':
                            vals_create["on_delete"] = 'cascade'
                        if param_id.type_param == 'many2many':
                            vals_create["relation_table"] = ''.join(
                                ['x_rew_', str(res.id), '_', str(param_id.id), '_', str(seq_number), '_rel'])
                    record = self.env['ir.model.fields'].create(vals_create)
                    vals_update['reports_designer_param_ids'].append(
                        [1, param_id.id, {'wizard_param_ir_model_field_id': record.id}])
    ...
    def write(self, vals):
        wizard_model_id = self.env['ir.model'].search([('model', '=', 'reports_designer_wizard')], []).id
        wizard_data_field_id = self.env['ir.model.fields'].search(
            [('name', '=', 'data'), ('model_id', '=', wizard_model_id)], []).id
        ...
        if 'reports_designer_param_ids' in vals:
            ...
            for param in vals['reports_designer_param_ids']:
                if param[0] in [1, 2, 3, 5]:
                    vals_update_before['reports_designer_param_ids'].append(
                        [1, param[1], {'wizard_param_ir_model_field_id': False}])
                    unlink_ir_model_fields_ids.append(
                        self.env['ir.model.fields'].search(
                            [('name', 'like', ''.join(['x_param_', str(self.id), '_', str(param[1])])),
                             ('model_id', '=', wizard_model_id)], []).id
                    )
            ...
            for param_id in self.reports_designer_param_ids:
                if param_id.wizard_param_ir_model_field_id.id == False:
                    ...
                    if param_id.param_ir_model_id.id != False:
                        ...
                        if param_id.type_param == 'many2many':
                            vals_create["relation_table"] = ''.join(
                                ['x_rew_', str(self.id), '_', str(param_id.id), '_', str(seq_number), '_rel'])
                    record = self.env['ir.model.fields'].create(vals_create)
                    vals_update['reports_designer_param_ids'].append(
                        [1, param_id.id, {'wizard_param_ir_model_field_id': record.id}])
            if len(vals_update['reports_designer_param_ids']):
        ...
        for section in res:
                    ...
                    if section.id != s.id:
                        ...
                        if s.id in section_intersection and section.id in section_intersection[s.id]:
                            ...
                            if len(result_intersection):
                                ...
                                ''' % (
                                section.name, section.section_start, section.section_end, s.name, s.section_start,
                                s.section_end, result_intersection_str,)
            ...
            for s in section.children_ids:
                ...
                if len(result_child_bound):
                    ...
                    ''' % (s.name, section.name, section.section_start, section.section_end, s.name, s.section_start,
                           s.section_end,)
                    msg_section = msg_section + msg
                    result = True
                    section_result = True
                ...
                if len(result_intersection):
                    ...
                    ''' % (
                    result_intersection_str, section.name, section.name, section.section_start, section.section_end,
                    s.name, s.section_start, s.section_end,)
                    msg_section = msg_section + msg
                    result = True
                    section_result = True
...
class ReportsDesignerParam(models.Model):
    ...
    param_ir_model_id = fields.Many2one('ir.model', ondelete='cascade', string='Param Model',
                                        domain=[('transient', '=', False)])
    wizard_param_ir_model_field_id = fields.Many2one('ir.model.fields', string='Wizard Param Model Field')
    param_required = fields.Boolean(string='Param Required',
                                    help="If checked, the Parameter becomes Mandatory for filling in the Wizard.",
                                    default=False)
...
class ReportsDesignerSection(models.Model):
    ...
    def _get_root_model_related_field_domain(self):
        if self._context.get('section'):
            res = self.env['ir.model.fields'].search([('model_id', '=', self._context.get('root_model_id')),
    ...
    def _check_root_model(self):
        ...
        if len(res):
            ...
            if not self._context.get('section'):
                raise UserError(
                    _('Report Root Model is not defined. Before you can define a Section, you must specify a Report Root Model.'))
            else:
                raise UserError(
                    _('Parent Section Field is not defined. Before you can define a Child Section, you must specify a Parent Section Field for this Section.'))
    ...
    report_id = fields.Integer(compute='_compute_report_id', string="Report id", default=_default_compute_report_id,
                               store=False)
    reports_designer_param_content = fields.Char(compute='_compute_reports_designer_param_content',
                                                 string="Report Parameters",
                                                 default=_default_reports_designer_param_content, store=False)
    ...
    reports_designer_fields_ids = fields.One2many('reports.designer.fields', 'reports_designer_section_id',
                                                  string='Report Excel Fields', copy=True)
    ...
    root_model_name = fields.Selection(selection='_list_all_models', string='Report Root Model Name',
                                       default=_compute_root_model_name)
    ...
    reports_designer_fields_sql_ids = fields.One2many('reports.designer.fields.sql', 'reports_designer_section_id',
                                                      string='Report Excel Fields SQL', copy=True)
    domain = fields.Char(string='Domain', default='[]',
                         help="The syntax for using a Report Parameter: param(PARAMETER CODE), for Example: param(ABC). Parameters are available for selection only after saving the Report!")
    archive_bool = fields.Boolean(string='Print Archived Data', default=False,
                                  help="By default, only Active Records are printed in the Report. \nIf you want Archived Records to be printed as well, check the box here.")
        ...
        if self._context.get('section'):
            ...
            if self.section:
                if self.root_model_id:
                    ...
                    self.root_model_name = False
                self.children_ids = [(2, line_id, False) for line_id in self.children_ids.ids]
                self.reports_designer_fields_ids = [(2, line_id, False) for line_id in
                                                    self.reports_designer_fields_ids.ids]
                self.domain = '[]'
    ...
    def _check_start_end(self):
        ...
        for section in self:
            ...
            if not row_start:
                msg = "There is no row 0 . Invalid Start Section coordinates: \"%s\",  Section: \"%s\" !" % (
                coord_string_start, section.name,)
                raise ValidationError(msg)
            ...
            row_end = int(row_end)
            if not row_end:
                msg = "There is no row 0 . Invalid End Section coordinates: \"%s\",  Section: \"%s\" !" % (
                coord_string_end, section.name,)
                raise ValidationError(msg)
            if row_start > row_end or CheckCell.column_index_from_string(
                    column_start) > CheckCell.column_index_from_string(column_end):
...
class ReportsDesignerFields(models.Model):
    ...
    def _check_root_model(self):
        ...
        if len(res):
            ...
            if not self._context.get('section'):
                raise UserError(
                    _('Parent Section Field is not defined. Before you can define a Report Section Fields, you must specify a Parent Section Field.'))
    ...
    reports_designer_section_id = fields.Many2one('reports.designer.section', string='Section', ondelete='cascade',
                                                  help="Report excel Section Reference.")
    ...
    aggregate_ids = fields.Many2many('reports.designer.aggregate', string='Aggregates',
                                     compute='_compute_aggregate_ids')
...
\u0023   result = cell(H7) ''',
    ...
    cumulative_model_field2 = fields.Char(compute='_compute_cumulative_model_field', string='Field ', readonly=True)
    ...
    ''')
    colOff = fields.Integer(string='Column Offset', default=0,
                            help="This value is used to specify the column offset within a cell in EMUs. EMUs (English Metric Units) are a high precision coordinate space (914400 dpi / 360000 dpc).")
    rowOff = fields.Integer(string='Row Offset', default=0,
                            help="This value is used to specify the row offset within a cell in EMUs. EMUs (English Metric Units) are a high precision coordinate space (914400 dpi / 360000 dpc).")
    cx = fields.Integer(string='Extent Length', default=1500000,
                        help="Specifies the length of the extents image in EMUs. This length shall dictate the size of the object as displayed (the result of any scaling to the original object). EMUs (English Metric Units) are a high precision coordinate space (914400 dpi / 360000 dpc).")
    cy = fields.Integer(string='Extent Width', default=1500000,
                        help="Specifies the width of the extents image in EMUs. This width shall dictate the size of the object as displayed (the result of any scaling to the original object). EMUs (English Metric Units) are a high precision coordinate space (914400 dpi / 360000 dpc).")
    ...
    def _compute_cumulative_model_field(self):
        for rec in self:
            ...
            try:
                for i in name_split:
                    ...
                    new_name = (new_name + ' --> ' + display_name + '(' + model + ')') if (new_name != '') else (
                                display_name + '(' + model + ')')
                    new_name2 = (new_name2 + ' --> ' + field_name + '(' + model + ')') if (new_name2 != '') else (
                                field_name + '(' + model + ')')
                    model = rel
            ...
            rec.cumulative_model_field2 = new_name2
    ...
    def _onchange_having_param_domain(self):
        ...
        if self.field_type in ('date',) and self.aggregate_id.code != 'count':
            return {'domain': {
                'having_param_id': [('reports_designer_id', '=', report_id), ('type_param', 'in', ('date',))]}}
        if self.field_type in ('date',) and self.aggregate_id.code == 'count':
            return {'domain': {'having_param_id': [('reports_designer_id', '=', report_id),
                                                   ('type_param', 'in', ('integer', 'float',))]}}
        elif self.field_type in ('datetime',) and self.aggregate_id.code != 'count':
            return {'domain': {
                'having_param_id': [('reports_designer_id', '=', report_id), ('type_param', 'in', ('datetime',))]}}
        elif self.field_type in ('datetime',) and self.aggregate_id.code == 'count':
            return {'domain': {'having_param_id': [('reports_designer_id', '=', report_id),
                                                   ('type_param', 'in', ('integer', 'float',))]}}
        else:
    ...
    @api.depends('having_operator', 'having_selection', 'having_param_id', 'having_value_type', 'having_value_float',
                 'having_value_date', 'having_value_datetime')
        ...
        for menu in self:
            ...
            action_id = self.env['ir.actions.act_window'].search(
                ["&", ('res_model', '=', 'reports_designer_wizard'), "|", ('context', '=', context_report1),
                 ('context', '=', context_report2)]).id
            if not action_id:
                reports_designer_name = self.env['reports.designer'].search(
                    [('id', '=', self.env.context.get('active_id'))]).name
