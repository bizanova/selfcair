...
from odoo.tools.safe_eval import datetime as s_datetime, dateutil as s_dateutil, json as s_json, time as s_time, \
    pytz as s_pytz
...
class ReportsDesignerGen(models.TransientModel):
    ...
    def create_xls(self, datas=None, cellutil=None):
        ...
        for record in records:
            try:
                f_name = safe_eval(res_conf['out_file_name'],
                                   {'obj': record, 'time': s_time, 'datetime': s_datetime.datetime})
                out_file_name = safe_eval(
                    "pytz.utc.localize(datetime.now(), is_dst=None).astimezone(tz).replace(tzinfo=None).strftime(f_name)",
                    {'obj': record, 'time': s_time, 'datetime': s_datetime.datetime, 'f_name': f_name, 'pytz': s_pytz,
                     'tz': tz})
                break
            except:
                _logger.info("Error or Old 'Output File Name' Expression Definition Syntax: %s  !",
                             res_conf['out_file_name'])
                out_file_name = pytz.utc.localize(datetime.now(), is_dst=None).astimezone(tz).replace(
                    tzinfo=None).strftime(res_conf['out_file_name'])
                break
        else:
            out_file_name = pytz.utc.localize(datetime.now(), is_dst=None).astimezone(tz).replace(tzinfo=None).strftime(
                res_conf['out_file_name'])
        ...
        if not xlsx_template.check_conf(conf_data['conf']):
            raise UserError(_('Template configuration Error. \nInvalid Sheet Name: \"%s\".\n' \
                              'Please check the Sheet Name in the template configuration, It must match the Name of the Sheet in the EXCEL Workbook.') % (
                                conf_data['conf']['sheet_reference'],))
        ...
        conf_data['conf'][
            'report_lang_code'] = res_conf.report_lang_id.code if res_conf.report_lang_id.code else self.env.context.get(
            'lang')
        ...
        for i in res_conf.reports_designer_param_ids:
            if i['wizard_param_ir_model_field_id']['name'] in datas['form'].get('data'):
                ...
                conf_data['conf']['report_params'][i['code']] = [i['id'], i['code'], i['type_param'],
                                                                 i['param_ir_model_id']['id'], False]
        conf_data['conf']['section'] = {}
        for i in res_conf.reports_designer_section_ids:
            conf_data['conf']['section'][i['id']] = self._get_section_conf(i['id'], conf_data['conf']['report_params'],
                                                                           xlsx_template, None, Drawing_ML)
        ...
        for s_id in conf_data['conf']['section']:
            conf_data['data'][s_id] = self._get_section_data(s_id, conf_data['conf'], chain_ids_tpl, xlsx_template, [],
                                                             None, {}, (), Models_Env, Fields_Env)
        ...
        for k, v in list(conf_data_out['conf']['section'].items()):
            ...
            conf_data_out['conf']['data_lines']['row_min'] = \
                self._get_conf_coordinate(xlsx_template, v['section_start'])[2] if \
                    self._get_conf_coordinate(xlsx_template, v['section_start'])[2] < \
                    conf_data_out['conf']['data_lines'][
                        'row_min'] else conf_data_out['conf']['data_lines']['row_min']
            conf_data_out['conf']['data_lines']['row_max'] = self._get_conf_coordinate(xlsx_template, v['section_end'])[
                2] if self._get_conf_coordinate(xlsx_template, v['section_end'])[2] > \
                      conf_data_out['conf']['data_lines']['row_max'] else conf_data_out['conf']['data_lines']['row_max']
            conf_data_out['conf']['data_lines']['col_min'] = \
                self._get_conf_coordinate(xlsx_template, v['section_start'])[1] if \
                    self._get_conf_coordinate(xlsx_template, v['section_start'])[1] < \
                    conf_data_out['conf']['data_lines'][
                        'col_min'] else conf_data_out['conf']['data_lines']['col_min']
            conf_data_out['conf']['data_lines']['col_max'] = self._get_conf_coordinate(xlsx_template, v['section_end'])[
                1] if self._get_conf_coordinate(xlsx_template, v['section_end'])[1] > \
                      conf_data_out['conf']['data_lines']['col_max'] else conf_data_out['conf']['data_lines']['col_max']
        for k, v in list(conf_data_out['conf']['data_lines']['section_boundaries'].items()):
            conf_data_out['conf']['data_lines']['section_boundaries'][k]['min'] = self._get_conf_coordinate(
                xlsx_template, conf_data_out['conf']['data_lines']['section_boundaries'][k]['min'])
            conf_data_out['conf']['data_lines']['section_boundaries'][k]['max'] = self._get_conf_coordinate(
                xlsx_template, conf_data_out['conf']['data_lines']['section_boundaries'][k]['max'])
            conf_data_out['conf']['data_lines']['section_boundaries'][k]['max'].append(
                conf_data_out['conf']['data_lines']['section_boundaries'][k]['max'][2])
            conf_data_out['conf']['data_lines']['section_max'] += (
                conf_data_out['conf']['data_lines']['section_boundaries'][k]['max'][0],)
        conf_data_out['conf']['data_lines']['section_max'] = tuple(
            set(conf_data_out['conf']['data_lines']['section_max']))
        if len(root_sections):
            root_sections_tmp = []
            for n in root_sections:
                root_sections_tmp.append((n[0], tuple(self._get_conf_coordinate(xlsx_template, n[1])),
                                          tuple(self._get_conf_coordinate(xlsx_template, n[2])),))
            f_get_item_1 = itemgetter(1)
            f_get_item_2 = itemgetter(2)
            root_sections_sorted = sorted(root_sections_tmp,
                                          key=lambda x: (f_get_item_2(f_get_item_1(x)), f_get_item_1(f_get_item_1(x)),))
                ...
                for c in range(conf_data_out['conf']['data_lines']['col_max'] + 1):
                    if c != 0 and r != 0:
                        res_data[r].append([[None, xlsx_template.get_column_letter(c), c, r, r],
                                            {'mergecell': False, 'style': '0', 'section_ids': (), 'present': False,
                                             'formula': False, 'pack': False, 'value': None}])
                    else:
                        if c != 0:
                            res_data[r].append([[None, xlsx_template.get_column_letter(c), c, r, r],
                                                {'mergecell': False, 'style': '0', 'section_ids': (), 'present': False,
                            ...
                            res_data[r].append([[None, None, c, r, r],
                                                {'mergecell': False, 'style': '0', 'section_ids': (), 'present': False,
                                                 'formula': False, 'pack': False, 'value': None}])
            row_idx = 0
            ...
            for col in res_data:
                for cell in col:
                    if cell[0][0] is not None and cell[1]['value'] is not None:
                        xlsx_template.write(conf_data_out['conf']['sheet_reference'],
                                            xlsx_template.cell_from_coordinate(cell[0][1], cell[0][3]),
                                            cell[1]['value'], cell_ext=cell, DrawingML=Drawing_ML.Pictures[cell[0][0]])
            xlsx_template.update_conf(conf_data_out['conf'], False, res_data)
            xlsx_template.shift_coordinate_row(conf_data_out['conf']['sheet_reference'], xlsx_conf['row_data_max'] + 1,
                                               len(res_data) - xlsx_conf['row_data_max'] - 1)
    ...
    def _get_section_conf(self, section_id, params=None, xlsx_template=None, chain_group=None, Drawing_ML=None):
        ...
        fields = self.env['reports.designer.fields'].sudo().search_read(
            [('id', 'in', section['reports_designer_fields_ids'])], [])
        for f in fields:
            if f['show']:
        ...
        conf_section = {
            ...
            'children_ids': {d['id']: d for d in (
                [self._get_section_conf(p, params, xlsx_template, chain_group, Drawing_ML) for p in
                 section['children_ids']] if len(section['children_ids']) else section['children_ids'])},
            ...
            'reports_designer_fields_ids': {d[0][0]: d for d in (
                [self._get_fields_conf(p, params, xlsx_template, Drawing_ML) for p in
                 section['reports_designer_fields_ids']] if len(section['reports_designer_fields_ids']) else section[
                    'reports_designer_fields_ids'])},
    ...
    def _get_domain(self, domain, params):
        ...
        if domain != '[]' and domain != False:
            domain_arr_0 = safe_eval(ustr(domain))
            domain_arr = []
            for i in domain_arr_0:
                if isinstance(i, tuple):
                    domain_arr.append(list(i))
                else:
                    domain_arr.append(i)
                ...
                while f.find('param(') > -1:
                    ...
                    if param:
                        if param[2] in ('char',):
                            ...
                            pv = 'datetime.strptime(\"' + param[4] + '\", \"%Y-%m-%d %H:%M:%S\")' if param[
                                4] else '\"\"'
    ...
    def _get_section_data(self, section_id, conf=None, active_ids=(), xlsx_template=None, order=[],
                          parent_data_line_id=None, res={}, chain_ids=(), Models_Env={}, Fields_Env={}):
        conf_active = self._get_active_conf(section_id, conf, order)
        domain_ext = expression.AND([conf_active['domain'], [('id', 'in', active_ids)]]) if len(active_ids) else \
            conf_active['domain']
        ...
        for child_section_id in conf_active['children_ids']:
            ...
            child_related_field_name.append(
                self.env['ir.model.fields'].browse(child_conf['root_model_related_field_id'][0]).name)
        res_root_lines = []
        for record in self.env[conf_active['root_model_name']].with_context(active_test=False).search(domain_ext) if \
                conf_active['archive_bool'] else self.env[conf_active['root_model_name']].search(domain_ext):
            rec_dict_tmp = {}
        ...
        for data_line in res_root_lines:
            ...
            data.append([[parent_data_line_id], (chain_ids_tpl,), [data_line['id']],
                         self._get_field_data(data_line, conf_active, xlsx_template, Models_Env, Fields_Env,
                                              conf['report_lang_code'])])
            for child_section_id in conf_active['children_ids']:
                child_conf = self._get_active_conf(child_section_id, conf, order)
                root_field_name = self._get_model(Models_Env, 'ir.model.fields', conf['report_lang_code'])[
                    child_conf['root_model_related_field_id'][0]].name
                root_active_ids = []
                ...
                self._get_section_data(child_section_id, conf, root_active_ids_tpl, xlsx_template, order,
                                       data_line['id'], res['children_ids'][child_section_id], chain_ids_tpl,
                                       Models_Env, Fields_Env)
        for d in data:
            res['data'].append(d)
        return res
    ...
    def _get_field_data(self, res, conf, xlsx_template, Models_Env, Fields_Env, lang):
        ...
        for k, v in list(conf['reports_designer_fields_ids'].items()):
            ...
            val[k] = [self._get_conf_coordinate(xlsx_template, k),
                      self._get_data(res['id'], conf['root_model_name'], field_split, Models_Env, Fields_Env, lang)]
        return val
        ...
        if len(field_split) > 1:
            ...
            if field['type'] == 'many2one':
                if res[field_split[0]]:
                    res_value = self._get_data(res[field_split[0]][0], field['relation'], field_split_ext, Models_Env,
                                               Fields_Env, lang)
                else:
                    res_value = ((None, '',),)
            else:
                if len(res[field_split[0]]):
                    ...
                    for p in res[field_split[0]]:
                        res_value = res_value + self._get_data(p, field['relation'], field_split_ext, Models_Env,
                                                               Fields_Env, lang)
                else:
                    res_value = ((None, '',),)
            return res_value
            ...
            if field['type'] == 'many2one':
                ...
                if res[field_split[0]]:
                    res_value = ((None, pytz.utc.localize(res[field_split[0]], is_dst=None).astimezone(
                        pytz.timezone(self._context.get('tz') or self.env.user.tz or 'UTC')).replace(tzinfo=None),),) if \
                        field['type'] == 'datetime' else ((None, res[field_split[0]],),)
                else:
                    res_value = ((None, '',),)
    ...
    def _post_processing_data(self, section_id, conf=None, data=None, parent_chain_ids=None, having_remove_ids=None):
            ...
            if v[4]['formula']:
                for dl in data_active['data']:
                    ...
                    for cell_val in dl[3][k][1]:
                        ...
                        while f.find('cell(') > -1:
                            ...
                            if cell != None:
                                ...
                                if type(ref_val) in (str,):
                                    ...
                                    rv = ("\'\'\'" + json.dumps(ref_val, default=json_serial) + "\'\'\'")
                                else:
                                    rv = ref_val
                                if type(ref_val) in (dict,):
                                    f = f[:i] + rv + f[ie + 1:]
                                else:
                                    f = f[:i] + str(rv) + f[ie + 1:]
                            else:
                        ...
                        try:
                            f = f.replace('.now()',
                                          '.now().replace(tzinfo=pytz.utc).astimezone(tz).replace(tzinfo=None)')
                            globaldict = {
                            ...
                            n_val = localdict['result'] if 'result' in localdict and type(localdict['result']) in (
                                int, float,) else 'result' in localdict and localdict['result'] or ''
        ...
        def get_vals(c, f, p):
            ...
            if len(f):
                ...
                if c != -1 and (lambda *args: len(f_get_item_1(*args)) > 1):
                    return lambda *args: str(
                        [x for x in [x for x in p if f_get_item_1(*args)[0][:c + 1] in x]] + [x for x in f_get_items(
                            f_get_item_3(*args))])
                else:
        ...
        if len(sort_by_group):
            ...
            def get_val_sort_group(f, ft):
                ...
                def permutation(t, ft):
                    ...
                    for n in t:
                        ...
                        n1 = MAP_FIELD_TYPE_BLANK[ft] if n[1] == '' and ft in (
                            'integer', 'float', 'monetary', 'date', 'datetime',) else n[1]
                        new_t.append((n1, n0,))
                    return tuple(new_t)
            ...
            for s in sort_by_group_sorted:
                get_val_sort_group_f = get_val_sort_group(s[0], conf_active['reports_designer_fields_ids'][s[0]][4][
                    'field_type'])
        ...
        if len(having):
            for rec in data_active_res:
                for h in having:
                    rec_val_h = MAP_FIELD_TYPE_BLANK[
                        conf_active['reports_designer_fields_ids'][h[0]][4]['field_type']] if rec[3][h[0]][1][0][
                                                                                                  1] == '' and \
                                                                                              conf_active[
                                                                                                  'reports_designer_fields_ids'][
                                                                                                  h[0]][4][
                    ...
                    try:
                        ...
                        if len(section_ids) > len(current_section_ids):
                            if current_section_ids == section_ids[:len(current_section_ids)]:
                                ...
                                r_mergecell, r_sect_ids = self._get_related_section(related_section_ids[-1], conf_data,
                                                                                    xlsx_template, Rels)
                            ...
                            if section_ids == current_section_ids:
                                if mergecell:
                                    ...
                                    r_mergecell, r_sect_ids = self._get_related_mergecell(section_ids, mergecell,
                                                                                          conf_data, xlsx_template,
                                                                                          Rels)
                                    related_mergecell += r_mergecell
                                    related_section_ids += r_sect_ids
                                    row_exist = False
                ...
                while row_exist:
                    try:
                        ...
                        if col not in cell_start:
                            ...
                            cell_start_ids_0 = (0,) + cell_start[col][1]['section_ids'] if cell_start[col][1][
                                                                                               'section_ids'] != (
                                                                                               0,) else \
                            cell_start[col][1][
                                'section_ids']
    ...
    def _get_start_section(self, section_id, conf_data=None, xlsx_template=None, Rels=None):
        ...
        row_min = data_lines['row_min']
        cols_range = ()
        for col in range(data_lines['section_boundaries'][section_id]['min'][1],
                         data_lines['section_boundaries'][section_id]['max'][1] + 1):
            cols_range += (col,)
        rows_range = ()
        for row in range(data_lines['section_boundaries'][section_id]['min'][2],
                         data_lines['section_boundaries'][section_id]['max'][2] + 1):
            rows_range += (row,)
                ...
                try:
                    ...
                    if len(section_ids_tmp_0) >= len(section_ids_0):
                        if section_ids_0[:-1] == section_ids_tmp_0[:len(section_ids_0) - 1]:
                            ...
                            r_mergecell, r_sect_ids = self._get_related_section(section_ids_tmp[:len(section_ids)],
                                                                                conf_data, xlsx_template, Rels)
                            related_mergecell += r_mergecell
                            related_section_ids += r_sect_ids
                            row_exist = False
                    else:
                        if section_ids_0[:-1] == section_ids_tmp_0:
                            ...
                            if mergecell:
                                related_mergecell += (mergecell,)
                                r_mergecell, r_sect_ids = self._get_related_mergecell(section_ids[:-1], mergecell,
                                                                                      conf_data, xlsx_template, Rels)
                                related_mergecell += r_mergecell
                                related_section_ids += r_sect_ids
                                row_exist = False
                ...
                try:
                    ...
                    if len(section_ids_tmp) > len(section_ids):
                        if section_ids == section_ids_tmp[:len(section_ids)]:
                            ...
                            r_mergecell, r_sect_ids = self._get_related_section(section_ids_tmp[:len(section_ids) + 1],
                                                                                conf_data, xlsx_template, Rels)
                            related_mergecell += r_mergecell
                            related_section_ids += r_sect_ids
                            row_exist = False
                        ...
                        if section_ids == section_ids_tmp:
                            if mergecell:
                                related_mergecell += (mergecell,)
                                r_mergecell, r_sect_ids = self._get_related_mergecell(section_ids, mergecell, conf_data,
                                                                                      xlsx_template, Rels)
                                related_mergecell += r_mergecell
                                related_section_ids += r_sect_ids
                                row_exist = False
            ...
            for ids in data_row[1]:
                ...
                if ids[:chain_group_ind] in parent_chain_ids:
                    if not data_row_idx_0:
                        ...
                        stack = {}
                        cell_pack = 0
                        parent_section_ids = section_boundaries[conf_active['id']]['section_chain_ids'][:-1] if len(
                            section_boundaries[conf_active['id']]['section_chain_ids'][:-1]) else (0,)
                        s_add_data_rows = section_boundaries[conf_active['id']]['max'][2] - \
                                          section_boundaries[conf_active['id']]['min'][2] + 1
                        for s_col in section_boundaries[conf_active['id']]['cols_range']:
                            ...
                            for s_row in section_boundaries[conf_active['id']]['rows_range']:
                                s_cells.append(dc(
                                    matrix_template[s_row - data_lines['row_min']][2][s_col - data_lines['col_min']]))
                            self._new_coordinate(s_cells, conf_data['conf'], xlsx_template, section_bottom_row + 1)
                            s_cells.extend(stack[s_col])
                            stack[s_col] = s_cells
                        self._shift_parent_sections(section_boundaries[conf_active['id']]['section_chain_ids'], stack,
                                                    s_add_data_rows, conf_data['conf'], xlsx_template, res_data)
                    ...
                    for item in conf_active['cell_section_order']:
                        if item[0]:
                            ...
                            if len(data_row[3][item[1]][1]) == 1:
                                ...
                                c_row_range = xlsx_conf['data_line_merge_cells'][m_cell[0][0]][4] if m_cell[1][
                                    'mergecell_cell'] else (self._get_conf_coordinate(xlsx_template, m_cell[0][0])[2],)
                                c_row_len = len(xlsx_conf['data_line_merge_cells'][m_cell[0][0]][4]) if m_cell[1][
                                    'mergecell_cell'] else 1
                                c_col_range = xlsx_conf['data_line_merge_cells'][m_cell[0][0]][3] if m_cell[1][
                                    'mergecell_cell'] else (m_cell[0][2],)
                                stack = {}
                                c_add_data_rows = (len(data_row[3][item[1]][1]) - 1) * len(c_row_range)
                                for c_col in c_col_range:
                                    stack[c_col], cell_pack = self._get_stack(c_col, m_cell[0][3] + c_row_len, res_data,
                                                                              m_cell[1]['section_ids'], c_add_data_rows)
                                    c_cells = []
                                    for value in data_row[3][m_cell[0][0]][1]:
                                        for c_row in c_row_range:
                                            c_cells.append(dc(matrix_template[c_row - data_lines['row_min']][2][
                                                                  c_col - data_lines['col_min']]))
                                            if c_cells[-1][0][0] == m_cell[0][0]:
                                                c_cells[-1][1]['value'] = value[1]
                                    c_cells.extend(stack[c_col])
                                    stack[c_col] = dc(c_cells)
                                for c_col, val in list(conf_active['reports_designer_fields_ids'][m_cell[0][0]][4][
                                                           'cell_start'].items()):
                                    m_idx = data_lines['matrix_cell_idx'][val[0][0]]
                                    c_row = matrix_template[m_idx[0]][m_idx[1]][m_idx[2]][0][3]
                                    stack[c_col], c_pack = self._get_stack(c_col, c_row, res_data,
                                                                           m_cell[1]['section_ids'], c_add_data_rows)
                                    c_pack_cells = []
                                    for n in range(c_add_data_rows - cell_pack + c_pack):
                                        ...
                                        c_pack_cells[-1][1]['section_ids'] = section_boundaries[conf_active['id']][
                                            'section_chain_ids']
                                ...
                                if (c_add_data_rows - cell_pack):
                                    self._shift_parent_sections(m_cell[1]['section_ids'], stack,
                                                                c_add_data_rows - cell_pack, conf_data['conf'],
                                                                xlsx_template, res_data)
                                self._write_stack(stack, conf_data['conf'], xlsx_template, res_data)
                        else:
    ...
    def _shift_parent_sections(self, section_ids, stack, row_shift, conf, xlsx_template=None, res_data=None):
        ...
        for k_test, stack_col_test in list(stack.items()):
            ...
            for cell_test in stack_col_test:
                ...
                section_ids_tmp_0_test = (0,) + section_ids_tmp_temp if section_ids_tmp_temp != (
                    0,) else section_ids_tmp_temp
                if section_ids_0_test[:-1] == section_ids_tmp_0_test and cell_pack_tmp_test < row_shift and \
                        cell_test[1]['pack']:
    ...
    def _get_order(self, section_id, conf=None, xlsx_template=None):
        ...
        child_sections_tmp = []
        for k, v in list(conf_active['children_ids'].items()):
            child_sections_tmp.append([(k, tuple(self._get_conf_coordinate(xlsx_template, v['section_start'])),
                                        tuple(self._get_conf_coordinate(xlsx_template, v['section_end'])),), None,
                                       None])
        ...
        conf_active['cell_section_order'] = sorted(cell_section_tmp, key=lambda x: (
            f_get_item_2(f_get_item_2(x)), f_get_item_1(f_get_item_2(x)),))
    ...
    def _get_active_conf(self, section_id, conf, order=None):
        if 'data_lines' in conf and 'section_boundaries' in conf['data_lines'] and section_id in conf['data_lines'][
            'section_boundaries']:
    ...
    def _get_model(self, Models_Env, model_name, lang=None):
        if model_name in Models_Env.models:
            ...
            if model_name in ('ir.model', 'ir.model.fields', 'ir.model.data',):
                model_ids = self.env[model_name].sudo().with_context(active_test=False, lang=lang).search([]) if \
                    self.env[model_name].sudo()._fields.get('active') else self.env[model_name].sudo().with_context(
                    lang=lang).search([])
            else:
                model_ids = self.env[model_name].with_context(active_test=False, lang=lang).search([]) if self.env[
                    model_name]._fields.get('active') else self.env[model_name].with_context(lang=lang).search([])
            model_ind = {}
            for r in model_ids:
                model_ind[r.id] = r
